  ...
  return (
    ...
    >
      <Image
        source={{
          uri:
            item.images?.[0]?.imageUrl || "https://via.placeholder.com/150x100",
        }}
...
const PlaceDetailsScreen = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      setRecommendations(
        sameCategoryPlaces.filter((p) => p.id !== parseInt(id))
      );
    } catch (err) {
  ...
  const handleReviewAdded = (reviewSubmitted) => {
    // Only refresh if a review was actually submitted
    if (reviewSubmitted) {
    ...
    // Always close the modal, regardless of submission
